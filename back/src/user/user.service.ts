...
export class UserService {
    ...
    public async  register(newUser: UserDto): Promise<string> {
        try {
            ...
            if(findedUser) { 
                throw "На данный email уже зарегистрирован пользователь"
            } 
            ...
            return token;
        } catch(err) {throw new HttpException("Что-то пошло не так", err.status)}
    }
    ...
    public async login(user: UserDto): Promise<string> {
        try {
            ...
            const isValidPass = await bcrypt.compare(user.password, findedUser.password);
            ...
            return token;
        } catch(err) {throw new HttpException("Что-то пошло не так", 500)}
    }
    ...
    public async newUserData(userLogin: string|jwt.JwtPayload, newUserData: UserDataDto): Promise<IUserData> { 
        try { 
            ...
            userData.save();
            return userData; 
        } catch(err) { console.log(err);throw new HttpException("Что-то пошло не так", 500)}    
    }

    public async getUserData(userLogin: string|jwt.JwtPayload): Promise<IUserData[]|false> { 
        try { 
            const userData = await this.userDataModel.find({user: userLogin}).exec();
    ...
    public async deleteUserData(userLogin: string|jwt.JwtPayload, id:  string): Promise<{success: boolean}> { 
        ...
        try {
            const userData = await this.userDataModel.findById(id).exec();
             if(userData.user != userLogin) {throw new Error}; // проверка на авторизацию
    ...
    public async editUserData(userLogin: string|jwt.JwtPayload, editedUserData: UserDataDto, id: string): Promise<{success: boolean}|IUserData> { 
        try {
            ...
            const userData = await this.userDataModel.findById(id).exec();
            if(userData?.user != userLogin) {throw new Error} // проверка на авторизацию
            await this.userDataModel.updateOne({_id: id}, { header: editedUserData.header,  body: editedUserData.body}).exec();
            return userData
        } catch(err) {console.log(err); throw new HttpException("Что-то пошло не так", 500)}
    }
